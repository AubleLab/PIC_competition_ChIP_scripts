...
# save(rld, file = "data/rld_PCA_pombeNormalized.Rdata")
load("data/rld_PCA_pombeNormalized.Rdata")
...
p = ggplot(plotData, aes(x=PC1, y=PC2, fill = fraction, color = added_4sU))
p + geom_point(size=6, alpha = 0.7, shape = 21, stroke = 1) +
  geom_point(data = plotData %>% dplyr::filter(timeInGal == "60"), color = "black") +
...
ggsave("figures/panels/figSa/PCA.pdf", width = 13, height = 6, units = "cm")
